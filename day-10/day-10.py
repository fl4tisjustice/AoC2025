...
def get_input() -> list[tuple[int, tuple[int, ...], tuple[int, ...]]]:
    ...
    for line in raw:
        ...
        buttons_raw : list[str] = list(map(re.Match[str].group, re.finditer(r"(?<=\()([0-9]+,)*[0-9]+(?=\))", line)))
...
def part_two(puzzle : list[tuple[int, tuple[int, ...], tuple[int, ...]]]) -> int:
    total_min_steps : float = 0.0
    for _, buttons, joltages in puzzle:
        A : list[list[int]] = [[int(((1 << i) & button) != 0) for button in buttons] for i in range(len(joltages) - 1, -1, -1)]
        total_min_steps += cast(float, sp.optimize.linprog([1] * len(buttons), A_eq = A, b_eq = joltages, integrality = 1).fun)
